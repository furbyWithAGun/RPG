...
int ZoneMap::manhattenDistance(int x1, int y1, int x2, int y2)
{
		...
		/*for (int j = 0; j < savedDooDads[i].attributes.size(); j++)
		{
			if (savedDooDads[i].attributes[j].attributeType == DOODAD_TYPE)
			{
				...
				case DOODAD_MINEBLOCK:
					returnVector.push_back(new MineBlock(savedDooDads[i].rawString, gameScene));
					break;
				default:
		...
		//{
			...
			/*if (savedBuildings[i].attributes[j].attributeType == BULDING_TYPE)
			{
				...
				case BUILDING_SMELTING_WORKS:
					returnVector.push_back(new SmeltingWorks(savedBuildings[i].rawString));;
					break;
				default:
